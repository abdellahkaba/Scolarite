    ...
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-2">
        <div class="flex justify-between items-center">
            ...
            <div class="w-1/3">
                ...
                <select name="selected_classe_id" id="" class="block mt-1 rounded-md border-gray-300 w-full" wire:model='selected_classe_id'>
                    <option value="">Filter par classe</option>
                    @foreach ($classList as $item)
                        <option value="{{ $item->id }}">{{ $item->libelle }}</option>
                    @endforeach
                </select>
            </div>
            <a href="{{ route('inscriptions.create') }}" class="text-white text-sm bg-blue-600 rounded-md p-2 m-2">Inscrire un elève</a>
        </div>
                            ...
                            <tbody>
                                @forelse ($inscriptions as $item)
                                    <tr class="border-b-2 border-gray-100">
                                        ...
                                        <td class="text-sm font-medium text-gray-900 px-6 py-6">
                                            {{ $item->student->contact_parents }}
                                        </td>
                                        <td class="text-sm font-medium text-gray-900 px-6 py-6">
                                            <a href="{{ route('inscriptions.show', $item->id) }}" class="text-sm bg-blue-900 p-1 text-white rounded-sm">Détail</a>
                                            <a href="{{ route('inscriptions.edit', $item->id) }}" class="text-sm bg-green-900 p-1 text-white rounded-sm">Modifier</a>  
                                        </td>
